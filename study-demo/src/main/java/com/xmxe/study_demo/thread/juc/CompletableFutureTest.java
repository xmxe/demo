...
public class CompletableFutureTest {
    ...
    public void thenApply() throws Exception {
         ...
         * thenApplyAsync与thenApply的区别在于，thenApplyAsync是将job2提交到线程池中异步执行，实际执行job2的线程可能是另外一个线程，
         ...
         * 不带Async的方法是由触发该任务的线程执行该任务，带Async的方法是由触发该任务的线程将任务提交到线程池，执行任务的线程跟触发任务的线程不一定是同一个。
         */
    ...
    // handle
    /**
     * 跟whenComplete基本一致，区别在于handle的回调方法有返回值，且handle方法返回的CompletableFuture的result时回调方法的执行结果或者回调方法执行期间抛出的异常,与原始CompletableFuture的result无关了。
     * 测试用例如下：
     */
