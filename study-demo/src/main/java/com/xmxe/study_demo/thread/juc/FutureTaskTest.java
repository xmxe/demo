...
public class FutureTaskTest {
    ...
    public static void main(String[] args) throws Exception{
        ...
        /**
         * NEW:初始状态 0
         * COMPLETING:任务已经执行完(正常或者异常)完成进行时，准备赋值结果 1
         * NORMAL:任务已经正常执行完，并已将任务返回值赋值到结果 2
         * EXCEPTIONAL:任务执行失败抛出异常，并将异常赋值到结果 3
         * CANCELLED:任务已取消 4
        ...
        /**
         * run方法重点做了以下几件事：
         * 将runner属性设置成当前正在执行run方法的线程，调用callable成员变量的call方法来执行任务
...
/**
 * calcel(boolean mayInterruptIfRunning)为中断任务，但是为什么设计的时候需要加入mayInterruptIfRunning参数来使正在进行的任务中断或者不中断继续执行呢，
 * 为什么不直接设计成一个无参的cancel()方法直接定义成中断正在执行的任务的方法？为什么还要设计一个不中断的方法参数
 * 经过测试cancel(false)得出自己的结论 不一定正确
 * 当使用cancel(false)的时候，Callable里面的方法会正常执行，但是不会返回结果了
 * 而使用cancel(true)的时候，Callable里面的方法就不会正常执行了，更不会返回结果了
