 ...
 * public final boolean compareAndSet(int expect, int update)  CAS实现 比较并交换 一种不依靠锁实现并发安全的技术 如果当前值 == 预期值，则以原子方式将该值设置为给定的更新值。参数：expect - 预期值   update - 新值 返回：如果成功，则返回 true。返回 False 指示实际值与预期值不相等。
 * public final boolean weakCompareAndSet(int expect,  int update) 与compareAndSet实现一样 只是针对不同平台的架构 如果当前值 == 预期值，则以原子方式将该设置为给定的更新值。可能意外失败并且不提供排序保证，所以只有在很少的情况下才对compareAndSet 进行适当地选择。 参数：expect - 预期值   update - 新值 返回：如果成功，则返回 true。
