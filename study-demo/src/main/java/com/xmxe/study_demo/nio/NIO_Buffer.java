 ...
 * FloatBuffer,IntBuffer, LongBuffer, ShortBuffer 分别对应基本数据类型: byte, char,
 * double, float,int, long, short。 NIO中还有MappedByteBuffer, HeapByteBuffer,
 * DirectByteBuffer等 缓冲区本质上是一块可以写入数据，然后可以从中读取数据的内存。这块内存被包装成NIO
 ...
 * 从Buffer中读取数据：从Buffer到Channel(channel.write(buf)) 使用get()方法从Buffer中读取数据
 * buf.get()
 */
public class NIO_Buffer{
    ...
    public void mappedByteBuffer(){
         ...
         * 
         * FileChannel中的几个变量：
         * MapMode mode：内存映像文件访问的方式，共三种：MapMode.READ_ONLY：只读，试图修改得到的缓冲区将导致抛出异常。MapMode.READ_WRITE：读/写，对得到的缓冲区的更改最终将写入文件；但该更改对映射到同一文件的其他程序不一定是可见的。MapMode.PRIVATE：私用，可读可写,但是修改的内容不会写入文件，只是buffer自身的改变，这种能力称之为”copy on write”。
         * position：文件映射时的起始位置。
     ...
     * position会向前移动到下一个可插入数据的Buffer单元。position最大可为capacity – 1.当读取数据时，也是从某个特定位置读。
     ...
     * limit:在写模式下，Buffer的limit表示你最多能往Buffer里写多少数据。 写模式下，limit等于Buffer的capacity。当切换Buffer到读模式时， 
