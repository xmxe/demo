...
public class NIO_Channel {
    ...
    public void newIORead() {
        ...
        try {
            ...
            while (bytesRead != -1) {
                // 在读模式下,可以读取之前写入到buffer的所有数据,调用flip()方法,position设回0,并将limit设成之前的position的值,数据就是从position到limit的数据
                ...
                // clear()方法：position将被设回0,limit设置成capacity,换句话说,Buffer被清空了,其实Buffer中的数据并未被清除,只是这些标记告诉我们可以从哪里开始往Buffer里写数据。
                // 如果Buffer中有一些未读的数据,调用clear()方法,数据将“被遗忘”,意味着不再有任何标记会告诉你哪些数据被读过,哪些还没有。
                // 如果Buffer中仍有未读的数据,且后续还需要这些数据,但是此时想要先写些数据,那么使用compact()方法。compact()方法将所有未读的数据拷贝到Buffer起始处。然后将position设到最后一个未读元素正后面。limit属性依然像clear()方法一样,设置成capacity。
    ...
    public void newIOSocketClient() {
        ...
        try {
            // 通过ServerSocketChannel.open()方法来创建一个新的ServerSocketChannel对象,
    ...
    public void NIOSocketServer() throws Exception {
        ...
        while (true) {
            ...
            // 在阻塞模式下当accept()方法返回的时候,它返回一个包含新进来的连接的SocketChannel,否则accept()方法会一直阻塞到有新连接到达。
            // 在非阻塞模式下,在没有新连接的情况下,accept()会立即返回null,该模式下通常不会仅仅只监听一个连接,因此需在while循环中调用accept()方法.
    ...
    public void DatagramChannelsend() throws Exception {
        ...
        // 如果传输队列没有足够空间来承载整个数据报,那么什么内容都不会被发送。请注意send()方法返回的非零值并不表示数据报到达了目的地,仅代表数据报被成功加到本地网络层的传输队列。
        // 此外,传输过程中的协议可能将数据报分解成碎片,被分解的数据报在目的地会被重新组合起来,接收者将看不到碎片。但是,如果有一个碎片不能按时到达,那么整个数据报将被丢弃。分解有助于发送大数据报,但也会会造成较高的丢包率。
    ...
    public void receiveData() throws IOException {
        ...
        ByteBuffer byteBuffer = ByteBuffer.allocate(1024);
        byteBuffer.clear();
        // 通过receive()方法接受DatagramChannel中数据。从该方法将传入的数据报的数据将被复制到ByteBuffer中,同时返回一个SocketAddress对象以指出数据来源。在阻塞模式下,receive()将会阻塞至有数据包到来,
