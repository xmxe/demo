...
public class CompletableFutureTest {
    public static void main(String[] args) throws Exception {
        ...
        // 重载版本,指定执行异步任务的Executor实现,如果不指定,默认使用ForkJoinPool.commonPool()
        completableFutureTest.supplyAsync2();
        // runAsync表示创建无返回值的异步任务,相当于ExecutorService submit(Runnable task)方法
        completableFutureTest.runAsync();
        // 重载版本,指定执行异步任务的Executor实现,如果不指定,默认使用ForkJoinPool.commonPool()
        completableFutureTest.runAsync2();
        // thenApply/thenApplyAsync异步回调
        completableFutureTest.thenApply();
        // thenAccept/thenRun:thenAccept同thenApply接收上一个任务的返回值作为参数,但是无返回值,thenRun的方法没有入参,也没有返回值
        completableFutureTest.thenAccept();
        // exceptionally方法指定某个任务执行异常时执行的回调方法,会将抛出异常作为参数传递到回调方法中,如果该任务正常执行,则会exceptionally方法返回的CompletionStage的result就是该任务正常执行的结果
        completableFutureTest.exceptionally();
        ...
        completableFutureTest.whenComplete();
        // handle跟whenComplete基本一致,区别在于handle的回调方法有返回值,且handle方法返回的CompletableFuture的result时回调方法的执行结果或者回调方法执行期间抛出的异常,与原始CompletableFuture的result无关了。
        completableFutureTest.handle();
        // thenCombine/thenAcceptBoth/runAfterBoth,这三个方法都是将两个CompletableFuture组合起来,只有这两个都正常执行完了才会执行某个任务,区别在于,
        ...
        // applyToEither/acceptEither/runAfterEither:这三个方法都是将两个CompletableFuture组合起来,只要其中一个执行完了就会执行某个任务,其区别在于
        // applyToEither会将已经执行完成的任务的执行结果作为方法入参,并有返回值
        // acceptEither同样将已经执行完成的任务的执行结果作为方法入参,但是没有返回值
        // runAfterEither没有方法入参,也没有返回值。
        // 注意两个任务中只要有一个执行异常,则将该异常信息作为指定任务的执行结果。
        completableFutureTest.applyToEither();
        ...
        // 如果该CompletableFuture实例为null,执行这个新任务
        completableFutureTest.thenCompose();
        // allOf/anyOf:allOf返回的CompletableFuture是多个任务都执行完成后才会执行,只有有一个任务执行异常,则返回的CompletableFuture执行get方法时会抛出异常,如果都是正常执行,则get返回null。
        // anyOf是只有一个任务执行完成,无论是正常执行或者执行异常,都会执行cf4,cf4.get的结果就是已执行完成的任务的执行结果
        completableFutureTest.allOf();
    ...
    /**
     * 创建一个有初始值CompletableFuture
     */
    static void completedFutureExample() {
        CompletableFuture<String> cf = CompletableFuture.completedFuture("message");
        Assert.isTrue(cf.isDone(), "cf.isDone() is not true");
        Assert.doesNotContain("message", cf.getNow(null), "doesNotContain");
    }
    ...
    // --- supplyAsync/runAsync这两方法的效果跟submit是一样的 ---
    ...
    public void thenApply() throws Exception {
         ...
         * thenApplyAsync与thenApply的区别在于,thenApplyAsync是将job2提交到线程池中异步执行,实际执行job2的线程可能是另外一个线程,thenApply是由执行job1的线程立即执行job2,即两个job都是同一个线程执行的。
         * 将上述测试用例中thenApply改成thenApplyAsync后,从输出可知,执行job1和job2是两个不同的线程。thenApplyAsync有一个重载版本,可以指定执行异步任务的Executor实现,如果不指定,默认使用ForkJoinPool.commonPool()。
    ...
    public void exceptionally() throws Exception {
        ...
        // cf执行异常时,将抛出的异常作为入参传递给回调方法,抛出异常后,只有cf2执行了,cf3没有执行。将上述示例中的if(true)改成if(false)进行测试:cf2没有指定,其result就是cf执行的结果,理论上cf2.get应该立即返回的,此处是等待了cf3,即job2执行完成后才返回
    ...
    public void thenCombine() throws Exception {
        ...
        // job1和job2几乎同时运行,job2比job1先执行完成,等job1退出后,job3和job4几乎同时开始运行,job4先退出,等job3执行完成,job5开始了,等job5执行完成后,主线程退出
    }
    ...
    public void applyToEither() throws Exception {
        ...
        // job1和job2同时开始运行,job2先执行完成,然后job4开始执行,理论上job3和job4应该同时开始运行,但是此时只有job4开始执行了,job3是等待job1执行完成后才开始执行,job4先于job3执行完成,然后job5开始执行,等job5执行完成后,主线程退出
    }
