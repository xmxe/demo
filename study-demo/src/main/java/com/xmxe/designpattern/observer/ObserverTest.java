package com.xmxe.designpattern.observer;
 ...
 * 没有线程安全.方法可以被其子类覆盖,并且事件通知可以以不同的顺序发生,并且可能在不同的线程上发生,这足以破坏任何“线程安全”。并且大多数问题都具有修复的复杂性,但仍然“未解决”或没有积极的发展,这就是不推荐使用的原因。
 * 观察者模式的替代方案:对于更丰富的事件模型,请考虑使用该java.beans程序包。为了在线程之间进行可靠且有序的消息传递,请考虑使用java.util.concurrent程序包中的并发数据结构之一有关反应式流样式的编程,请参见Flow API。
 */
